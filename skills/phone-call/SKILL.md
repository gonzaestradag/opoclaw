---
name: phone-call
description: Make AI phone calls using Gonzalo's cloned voice. Triggers on: "llama a", "llámale a", "habla con", "confirma la reserva", "call this place", "make a call to", "llama al restaurante", "márcale a".
allowed-tools: Bash
---

# phone-call

Make outbound AI phone calls using Vapi.ai with Gonzalo's ElevenLabs cloned voice.

## Workflow

1. **Resolve who to call** — if Gonzalo gives a name (person or place), NOT a number:
   - For a **person**: query contacts DB first:
     ```bash
     sqlite3 /Users/opoclaw1/claudeclaw/store/claudeclaw.db "SELECT name, phone FROM contacts WHERE name LIKE '%NAME%' LIMIT 3;"
     ```
   - For a **business/place**: search the web for the phone number using WebSearch tool
2. **Confirm with Gonzalo** — reply with just the name and what you'll say. Never show the number. Example:
   - "Marco a Azael para decirle que mañana no puedes."
   - "Llamo a La Paloma para reservar mesa para 4 esta noche a las 8."
3. **Make the call** once Gonzalo confirms
4. **Report back** — just confirm the call was initiated. When the call ends, Gonzalo receives an automatic AI-generated voice note summarizing what was said and what was accomplished. No manual transcript check needed.

## Make a call

```bash
VAPI_KEY=$(grep VAPI_API_KEY /Users/opoclaw1/claudeclaw/.env | cut -d= -f2)
PHONE_ID=$(grep VAPI_PHONE_NUMBER_ID /Users/opoclaw1/claudeclaw/.env | cut -d= -f2)
...
```
Eres el asistente virtual de Gonzalo Estrada. Hoy es [TODAY].
REGLA OBLIGATORIA: Al inicio de CADA llamada, preséntate exactamente así:
"Hola, soy el asistente virtual de Gonzalo Estrada."
...
- Always introduce as "el asistente virtual de Gonzalo Estrada" — this is non-negotiable on every call
...
- If voicemail: leave the intro + message + ask them to call Gonzalo back
...
2. Synthesizes it as a voice note using Gonzalo's ElevenLabs voice
3. Sends the voice note directly to Gonzalo via Telegram

If TTS fails, it sends the text summary. If the call had no transcript (no answer, voicemail), it sends a plain status message.

**No manual follow-up needed.** Gonzalo gets the result the same way he gets any other Thorn response.
...
- Voice ID: 06RLvYUqE7ke4HDI57RQ (Gonzalo's clone)
- Contacts DB: sqlite3 /Users/opoclaw1/claudeclaw/store/claudeclaw.db (table: contacts)
- Always confirm by NAME before calling — never show the number to Gonzalo
