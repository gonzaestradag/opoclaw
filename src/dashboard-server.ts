...
async function processOneEmailAI(db: Database.Database, msg: any): Promise<void> {
  ...
  const prompt = `You are Finn, an AI email assistant managing opoclaw@gmail.com for Gonzalo Estrada, CEO of OpoClaw — an AI automation agency. Analyze this email and return ONLY valid JSON with no markdown, no explanation, no code fences.
...
{"category":"<one of 8 categories>","summary":"<1-2 sentence summary of what this email is about and what action if any is needed>","draft":"<if to_respond: write a direct professional reply on behalf of Gonzalo at OpoClaw. Be concise, no emojis, no corporate fluff, match the tone of the email. Sign off as Gonzalo, OpoClaw. Otherwise: null>"}`;
...
<body>
  ...
  app.post('/api/auth/login', (req: Request, res: Response) => {
    ...
    const storedUsername = process.env.DASHBOARD_USERNAME || '';
    const dashToken = process.env.DASHBOARD_TOKEN || 'thorn2026';
  ...
  app.get('/api/metrics', (_req: Request, res: Response) => {
    try {
      ...
      const taskCounts = db.prepare(`
        SELECT
          COUNT(*) as total,
          SUM(CASE WHEN lower(status) = 'done' THEN 1 ELSE 0 END) as completed,
  ...
  app.post('/api/briefs/generate', (_req: Request, res: Response) => {
    try {
      ...
      const generatePrompt = [
        'Genera el Morning Podcast diario para Gonzalo. Pasos:',
  ...
  // Returns {allowed, known, isGonzalo} based on the VAPI_ALLOWED_CALLERS whitelist
  // and the GONZALO_VERIFIED_PHONE env var.
  // isGonzalo = true only when the caller number exactly matches GONZALO_VERIFIED_PHONE.
  function checkCallerIdentity(callerNumber: string): { allowed: boolean; known: boolean; name: string | null; isGonzalo: boolean } {
    ...
    // Check if this is Gonzalo's verified number
    const gonzaloPhone = (process.env['GONZALO_VERIFIED_PHONE'] ?? '').replace(/[\s\-().]/g, '');
    const isGonzalo = !!gonzaloPhone && (
      cleanNumber === gonzaloPhone ||
      cleanNumber.endsWith(gonzaloPhone.slice(-10)) ||
      gonzaloPhone.endsWith(cleanNumber.slice(-10))
    );
    ...
    // If caller is Gonzalo, use "Gonzalo" as display name
    const name = isGonzalo ? 'Gonzalo' : (contact?.name ?? null);
    ...
    try {
      ...
      if (msgType === 'assistant-request') {
        ...
        if (identity.isGonzalo) {
          // ── Gonzalo verified: full Thorn access, identical to Telegram ──
          logger.info('[vapi/inbound] Gonzalo verified — granting full Thorn access');

          // Load Gonzalo's memory context fresh on every call (same as Telegram bot does)
          // Salience-first ordering surfaces the most important memories, then recency.
          const gonzaloChatId = process.env['ALLOWED_CHAT_ID'] ?? '7363084116';
          ...
          const voiceSystemPrompt = claudeMdContent
            ? `${claudeMdContent}\n\n---\n\n## PHONE CALL CONTEXT\n\nYou are now on a PHONE CALL with Gonzalo. Everything above applies — same personality, same rules, same access. Same Thorn as in Telegram.\n\nAdapt for voice:\n- Keep responses short and spoken-friendly. No bullet points, no markdown, no lists unless asked.\n- One clear answer per turn.\n- Today is ${dateStr}, current time is ${timeStr}. Gonzalo is calling from his verified number.${callToolBlock}${memoryBlock}`
            : `You are Thorn, Gonzalo Estrada's personal assistant and COO of OpoClaw. You are on a phone call. Today is ${dateStr}, current time is ${timeStr}. Gonzalo is calling from his verified number.\n\nPersonality rules:\n- No emojis. Ever.\n- No em dashes.\n- No AI cliches.\n- Talk like a real person — direct, no filler.\n- Keep voice responses short and spoken-friendly.\n- One clear answer per turn.${callToolBlock}${memoryBlock}`;

          // Build the tools list for Gonzalo's assistant — all tools implemented in /api/vapi
          const gonzaloTools = [
            {
              type: 'function',
        ...
        // ── Unknown caller: intelligent assistant — takes messages, creates tasks, notifies Gonzalo ──
        logger.info({ callerNumber, callerDisplay }, '[vapi/inbound] Unknown caller — smart message-taking mode');
        const dashboardUrlStranger = process.env['DASHBOARD_URL'] || 'https://keisha-inescapable-clavately.ngrok-free.dev';
        return res.json({
          assistant: {
            firstMessage: 'Hola, soy el asistente de Gonzalo Estrada. En que le puedo ayudar?',
            model: {
              provider: 'anthropic',
              model: 'claude-haiku-4-5',
              messages: [{
                role: 'system',
                content: `Eres el asistente de Gonzalo Estrada. Hoy es ${dateStr}. NO eres Thorn. Tu funcion: ayudar al visitante y siempre notificar a Gonzalo.\n\nReglas:\n- Habla en espanol natural, sin emojis, sin frases de AI.\n- Jamas des informacion personal, de agenda, financiera o de negocio de Gonzalo.\n- Si alguien pide hablar con Gonzalo directamente, di que no esta disponible en este momento.\n\nFlujos que debes manejar:\n1. Si quiere dejar un mensaje: pregunta su nombre y el mensaje. Luego llama send_message_to_gonzalo. Luego llama create_task con un titulo como "Mensaje de [nombre]: [resumen]". Confirma al visitante que el mensaje fue enviado.\n2. Si pide que Gonzalo le devuelva la llamada: pregunta su nombre y numero. Llama send_message_to_gonzalo con el numero incluido. Confirma que notificaste a Gonzalo.\n3. Cualquier otro asunto: toma el nombre y asunto, llama send_message_to_gonzalo, confirma.\n\nSiempre al final: confirma al visitante que Gonzalo sera notificado.`,
              }],
              tools: [
                {
                  type: 'function',
  ...
  app.post('/api/vapi/webhook', async (req: Request, res: Response) => {
    try {
      ...
      let callRow = vapiCallId
        ? db.prepare(`SELECT id FROM calls WHERE vapi_call_id = ? LIMIT 1`).get(vapiCallId) as { id: string } | undefined
        : undefined;
      if (!callRow) {
        callRow = db.prepare(`SELECT id FROM calls WHERE status = 'queued' ORDER BY created_at DESC LIMIT 1`).get() as { id: string } | undefined;
      }
...
Today is ${new Date().toISOString().split('T')[0]}. Use 24h time. If no date specified, use today. If no time specified, use 09:00. If no duration, default 60 minutes. Extract ALL email addresses mentioned in the request and put them in the attendees array. If no emails are mentioned, return an empty array for attendees.`;
  ...
  async function pollProspectReplies(): Promise<void> {
    ...
    for (const { id: gmailId } of messages) {
      ...
      const replyPrompt = `You are Maya Chen, Operations Manager at OpoClaw — an AI automation agency run by Gonzalo Estrada. You are managing the business email opoclaw@gmail.com and responding to a prospect who replied to our cold outreach.
...
Return ONLY the email body text (no subject line, no JSON, no explanation).`;
            ...
            if (toolName === 'check_calendar') {
              ...
              if (!name) { result = 'Falta el nombre del contacto.'; }
                ...
                if (contacts.length === 0) {
                  result = `No encontré a "${name}" en los contactos de Gonzalo.`;
                } else {
                  ...
                  body: JSON.stringify({
                    ...
                    assistantOverrides: {
                      ...
                      model: {
                        messages: [{
                          role: 'system',
                          content: `Eres Thorn, COO de OpoClaw, llamando en nombre de Gonzalo Estrada. Hoy es ${today}. Tu tarea en esta llamada: ${task}. Habla en español natural y directo — sin rodeos, sin clichés de AI, sin emojis. Cuando termines la llamada, di exactamente lo que respondió la persona.`,
                        }],
                      },
                    },
                  }),
  ...
  app.post('/v1/chat/completions', async (req: Request, res: Response) => {
    try {
      ...
      const token = process.env['DASHBOARD_TOKEN'] || 'thorn2026';
