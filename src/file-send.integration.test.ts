...
function createTempFile(filename: string, content: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'opoclaw-test-'));
...
describe('file sending: real Telegram API', () => {
  ...
  beforeEach(() => {
    tmpFile = createTempFile('opoclaw-test.txt', `OpoClaw file send test\nTimestamp: ${new Date().toISOString()}\nThis file was sent by an automated integration test.`);
  });
  ...
  it.skipIf(!canRunRealTests)('sends a real document via Telegram sendDocument API', async () => {
    ...
    formData.append('document', new Blob([fs.readFileSync(tmpFile)]), 'opoclaw-test.txt');
    ...
    expect(json.result?.document?.file_name).toBe('opoclaw-test.txt');
  }, 15000);
...
%%EOF`;
    ...
    const pdfFile = createTempFile('opoclaw-test.pdf', pdfContent);
    try {
      const formData = new FormData();
      formData.append('chat_id', chatId);
      formData.append('document', new Blob([fs.readFileSync(pdfFile)]), 'opoclaw-test.pdf');
      ...
      expect(json.ok).toBe(true);
      expect(json.result?.document?.file_name).toBe('opoclaw-test.pdf');
    } finally {
  ...
  it.skipIf(!canRunRealTests)('returns error for nonexistent file (sanity check)', async () => {
    ...
    const fakePath = '/tmp/opoclaw-nonexistent-test-file.pdf';
