...
async function main() {
  ...
  console.log(`  ${c.bold}Welcome to OpoClaw.${c.reset}`);
  console.log();
  ...
  // ── 2. What is OpoClaw ────────────────────────────────────────────────
  section('What is OpoClaw?');

  console.log(`  OpoClaw bridges your Claude Code CLI to Telegram.`);
  console.log(`  You message your bot from your phone. OpoClaw runs the`);
  ...
  info('OpoClaw has several optional features. Tell us what you want.');
  ...
  if (wantWhatsApp) {
    ...
    console.log();
    info('OpoClaw uses whatsapp-web.js to connect to your existing WhatsApp');
  ...
  info('OpoClaw is one of several "Claw" projects. You might want to');
  ...
  info('OpoClaw auto-loads every skill in ~/.claude/skills/.');
  ...
  if (wantVideo) {
    ...
    console.log();
    info('OpoClaw\'s video analysis uses the gemini-api-dev skill from Google.');
  ...
  info('By default, OpoClaw uses your existing claude login (Max plan).');
  ...
  const lines = [
    '# OpoClaw — generated by setup wizard',
  ...
  if (PLATFORM === 'darwin') {
    ...
    info('Or use PM2: pm2 start dist/index.js --name opoclaw && pm2 save');
  }
  ...
  console.log(`  ${c.cyan}║${c.reset}${c.bold}           OpoClaw is ready!             ${c.reset}${c.cyan}║${c.reset}`);
  console.log(`  ${c.cyan}╚════════════════════════════════════════════╝${c.reset}`);
  console.log();
  ...
  console.log();
  if (PLATFORM === 'darwin') {
    info('Logs: tail -f /tmp/opoclaw.log');
  } else if (PLATFORM === 'linux') {
    info('Logs: journalctl --user -u opoclaw -f');
  }
  console.log();
...
async function setupMacOS() {
  ...
  const dest = path.join(os.homedir(), 'Library', 'LaunchAgents', 'com.opoclaw.app.plist');
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.opoclaw.app</string>
  ...
  <key>StandardOutPath</key><string>/tmp/opoclaw.log</string>
  <key>StandardErrorPath</key><string>/tmp/opoclaw.err</string>
...
async function setupLinux() {
  ...
  if (!install) {
    ...
    info('Or: pm2 start dist/index.js --name opoclaw && pm2 save');
    return;
  }
  ...
  try {
    ...
    const servicePath = path.join(serviceDir, 'opoclaw.service');
    const service = `[Unit]
Description=OpoClaw Telegram Bot
...
`;
    ...
    execSync('systemctl --user enable opoclaw', { stdio: 'pipe' });
    execSync('systemctl --user start opoclaw', { stdio: 'pipe' });
...
function setupWindows() {
  ...
  info('  Install WSL2, clone OpoClaw inside the WSL2 filesystem,');
  ...
  console.log(`  ${c.cyan}pm2 start dist/index.js --name opoclaw${c.reset}`);
